...
acc_facet_plot_main <- ggplot(acc_melt, aes(x = Method, y = value, fill = Method)) +
  geom_bar(
    stat = "identity", width = 0.8, color = "black"
  ) +
  geom_text(aes(label = round(value, 3)),
...
ggsave(
  file = paste0(figs_wd, "ACC_control_pou_0.05_7methods_facet_plot_nolegend_main.png"),
  plot = acc_facet_plot_main, width = 5.5, height = 5, units = "in", dpi = 200
)
