...
annotation_wd <- paste0(ana_wd, "annotation_results_scenario2/")

seu_name_scenario2 <- paste0(ana_wd, "seu_fov", fov, "_scenario2.rda")
load(seu_name_scenario2)
...
res_anno_measures_scenario2 <- list(
    acc_pou = res_acc_pou,
...
save(res_anno_measures_scenario2, file = paste0(ana_wd, "res_anno_measures_fov", fov, "_scenario2.rda"))



## ASW
res_asw <- c(
    CAESAR = asw(Embeddings(seu, "caesar"), seu$merged_clusters),
    CelliD = asw(Embeddings(seu, "mca"), seu$merged_clusters)
)


## SigScore
load(paste0(work_wd, ana_folder, "/degs.rda"))
result <- degs %>%
...
res_SigScore <- c(
    CAESAR = SigScore(seu, "caesar", "merged_clusters", result),
    CelliD = SigScore(seu, "mca", "merged_clusters", result)
)

res_measures_scenario1 <- list(
    asw = res_asw,
    SigScore = res_SigScore,
    n = table(seu$merged_clusters)
)

save(res_measures_scenario1, file = paste0(ana_wd, "res_measures_fov", fov, "_scenario2.rda"))
...
save(res_anno_measures_scenario2, file = paste0(ana_wd, "res_anno_measures_control_pou_0.05_fov", fov, "_scenario2.rda"))
