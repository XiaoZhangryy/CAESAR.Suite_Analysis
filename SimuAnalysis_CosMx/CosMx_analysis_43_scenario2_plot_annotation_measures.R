...
acc_melt <- Reduce(rbind, lapply(2:4, function(i, methods) {
    ...
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "fov", qfov, "/res_anno_measures_fov", qfov, "_scenario2.rda")
        load(res_file)
        res1 <- t(sapply(res_anno_measures_scenario2$acc_pou, function(dat) dat["ACC", methods]))
        res1 <- cbind(res_anno_measures_scenario2$acc_pou_iCAESAR["ACC", "iCAESAR_ref1"], res1)
        res1
    })
    names(measures) <- fovs
...
save(acc_melt, file = paste0(figs_wd, "scenario2_acc_7methods_melt.rda"))
...
ggsave(
    file = paste0(figs_wd, "scenario2_ACC_7methods_facet_plot.png"),
...
f1_melt <- Reduce(rbind, lapply(2:4, function(i, methods) {
    ...
    measures <- lapply(fovs, function(qfov) {
        ...
        n_cts <- res_anno_measures_scenario2$n

        res1 <- t(sapply(res_anno_measures_scenario2$f1score, function(dat) {
        ...
        F1res <- res_anno_measures_scenario2$f1score_iCAESAR[["iCAESAR"]]
        nn <- n_cts[colnames(F1res)]
        nn <- nn / sum(nn)
...
save(f1_melt, file = paste0(figs_wd, "scenario2_f1_7methods_melt.rda"))
...
ggsave(
    file = paste0(figs_wd, "scenario2_F1_7methods_facet_plot.png"),
...
pou_melt <- Reduce(rbind, lapply(2:4, function(i, methods) {
    ...
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "fov", qfov, "/res_anno_measures_fov", qfov, "_scenario2.rda")
        load(res_file)
        res1 <- t(sapply(res_anno_measures_scenario2$acc_pou, function(dat) dat["POU", paste0(methods, "unasg")]))
        res1 <- cbind(res_anno_measures_scenario2$acc_pou_iCAESAR["POU", "iCAESARunasg_ref1"], res1)
        res1
    })
    names(measures) <- fovs
    measures_array <- abind(measures, along = 3)
...
save(pou_melt, file = paste0(figs_wd, "scenario2_pou_7methods_melt.rda"))
...
ggsave(
    file = paste0(figs_wd, "scenario2_POU_7methods_facet_plot.png"),
...
acc_melt <- Reduce(rbind, lapply(2:4, function(i, methods) {
    ...
    load(paste0(work_wd, "processed_data_CosMx", i, "/fovs.rda"))
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "fov", qfov, "/res_anno_measures_control_pou_0.05_fov", qfov, "_scenario2.rda")
        load(res_file)
        res1 <- t(sapply(res_anno_measures_scenario2$acc_pou, function(dat) dat["ACC", methods]))
        res1 <- cbind(res_anno_measures_scenario2$acc_pou_iCAESAR["ACC", "iCAESAR"], res1)
        res1
    })
    names(measures) <- fovs
    measures_array <- abind(measures, along = 3)
...
save(acc_melt, file = paste0(figs_wd, "scenario2_acc_control_pou_0.05_7methods_melt.rda"))
...
ggsave(
    file = paste0(figs_wd, "scenario2_ACC_control_pou_0.05_7methods_facet_plot.png"),
    plot = acc_facet_plot, width = 12.5, height = 2.5, units = "in", dpi = 200
)
...
save(f1_melt, file = paste0(figs_wd, "scenario2_f1_control_pou_0.05_7methods_melt.rda"))
...
ggsave(
    file = paste0(figs_wd, "scenario2_F1_control_pou_0.05_7methods_facet_plot.png"),
    plot = f1_facet_plot, width = 12.5, height = 2.5, units = "in", dpi = 200
)
...
pou_melt <- Reduce(rbind, lapply(2:4, function(i, methods) {
    ...
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "fov", qfov, "/res_anno_measures_control_pou_0.05_fov", qfov, "_scenario2.rda")
        load(res_file)
        res1 <- t(sapply(res_anno_measures_scenario2$acc_pou, function(dat) dat["POU", methods]))
        res1 <- cbind(res_anno_measures_scenario2$acc_pou_iCAESAR["POU", "iCAESAR"], res1)
        res1
    })
    names(measures) <- fovs
    measures_array <- abind(measures, along = 3)
...
save(pou_melt, file = paste0(figs_wd, "scenario2_pou_control_pou_0.05_7methods_melt.rda"))
...
ggsave(
    file = paste0(figs_wd, "scenario2_POU_control_pou_0.05_7methods_facet_plot.png"),
    plot = pou_facet_plot, width = 12.5, height = 2.5, units = "in", dpi = 200
)
...
asw_melt <- Reduce(rbind, lapply(2:4, function(i) {
    ...
    load(paste0(work_wd, "processed_data_CosMx", i, "/fovs.rda"))
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "fov", qfov, "/res_measures_fov", qfov, "_scenario2.rda")
        load(res_file)
        
        res_measures_scenario2$asw
    })
    names(measures) <- fovs
...
save(asw_melt, file = paste0(figs_wd, "scenario2_asw_melt.rda"))
...
ggsave(
    file = paste0(figs_wd, "scenario2_ASW_facet_plot.png"),
...
sigscore_melt <- Reduce(rbind, lapply(2:4, function(i) {
    ...
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "fov", qfov, "/res_measures_fov", qfov, "_scenario2.rda")
        load(res_file)
        res_measures_scenario2$SigScore
    })
    names(measures) <- fovs
    measures_array <- abind(measures, along = 2)
    dimnames(measures_array) <- list(
...
save(sigscore_melt, file = paste0(figs_wd, "scenario2_sigscore_melt.rda"))
...
ggsave(
    file = paste0(figs_wd, "scenario2_sigscore_facet_plot_plot.png"),
