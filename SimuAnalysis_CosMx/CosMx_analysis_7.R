...
acc_melt <- Reduce(rbind, lapply(2:4, function(i) {
    ...
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "measures_fov", qfov, "_bin5.rda")
        load(res_file)
...
save(acc_melt, file = paste0(work_wd, "Figs_all/acc_melt_bin5.rda"))
...
ggsave(
    file = paste0(figs_wd, "ACC_facet_plot_bin5.png"),
...
asw_melt <- Reduce(rbind, lapply(2:4, function(i) {
    ...
    load(paste0(work_wd, "processed_data_CosMx", i, "/fovs.rda"))
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "embedding_measures_fov", qfov, "_bin5.rda")
        load(res_file)
        res_asw
    })
    names(measures) <- fovs
...
save(asw_melt, file = paste0(work_wd, "Figs_all/asw_melt_bin5.rda"))
...
sigscore_melt <- Reduce(rbind, lapply(2:4, function(i) {
    ...
    load(paste0(work_wd, "processed_data_CosMx", i, "/fovs.rda"))
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "coembedding_measures_fov", qfov, "_bin5.rda")
        load(res_file)
        res_coembed_score
    })
    names(measures) <- fovs
...
save(sigscore_melt, file = paste0(work_wd, "Figs_all/sigscore_melt_bin5.rda"))
