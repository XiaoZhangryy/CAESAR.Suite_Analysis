...
acc_melt <- Reduce(rbind, lapply(2:4, function(i) {
    ...
    measures <- lapply(fovs, function(qfov) {
        res_file <- paste0(ana_wd, "measures_fov", qfov, "_rbin5.rda")
        load(res_file)
...
save(acc_melt, file = paste0(work_wd, "Figs_all/acc_melt_rbin5.rda"))
...
ggsave(
    file = paste0(figs_wd, "ACC_facet_plot_rbin5.png"),
