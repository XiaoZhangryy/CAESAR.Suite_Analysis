...
################################################################################
# 2. co-embedding plot w.r.t. markerlist
################################################################################
load("sgInt.rda")
load("ct_ratio.rda")
load("seu_sc.rda")
...
for (i in 1:4) {
    ...
    ggsave(
        file = paste0(ana_wd, "CoUMAP_CAESARunasg_HCC", i, "_marker_blank.png"),
...
ggsave(
    file = paste0(figs_wd, "sc_CoUMAP_CAESARunasg_marker_blank.png"),
    plot = plot4, width = 4.2, height = 3.5, units = "in", dpi = 200
)
